...
* <https://github.com/proxy-py/proxy-py-docker>
...
helm install proxy-py nas-helm-charts/proxy-py --set AKAUNTING_SETUP="true"
```

## Installing the Chart

To install the chart with the release name `proxy-py`
...
To uninstall the `proxy-py` deployment

```console
helm uninstall proxy-py
```
...
```console
helm install proxy-py \
  --set env.TZ="UTC" \
    nas-helm-charts/proxy-py
```

Alternatively, a YAML file that specifies the values for the above parameters can be provided while installing the chart.

```console
helm install proxy-py nas-helm-charts/proxy-py -f values.yaml
```
...
|-----|------|---------|-------------|
| controller.main.containers.main.env | object | See below | environment variables. See more environment variables in the [proxy-py documentation](https://proxy-py.org/docs). |
| controller.main.containers.main.env.TZ | string | `"UTC"` | Set the container timezone |
| image.pullPolicy | string | `"IfNotPresent"` | image pull policy |
| image.repository | string | `"proxy-py/proxy-py"` | image repository |
